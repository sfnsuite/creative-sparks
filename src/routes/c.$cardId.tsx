...
const cardQuery = (id: string) => queryOptions({
  queryKey: ["promo", id],
  queryFn: async () => {
    const { data: card, error } = await supabase.from("promo_cards").select("*").eq("id", id).eq("active", true).maybeSingle();
    if (error) throw error;
    if (!card) throw notFound();
    const ids = [card.active_product_id, ...(card.companion_product_ids ?? [])].filter(Boolean) as string[];
    let products: Product[] = [];
    if (ids.length) {
      const { data: p, error: pe } = await supabase.from("products").select("*").in("id", ids).eq("active", true);
      if (pe) throw pe;
      products = p ?? [];
    }
...
export const Route = createFileRoute("/c/$cardId")({
  head: ({ loaderData }: { loaderData?: { card: PromoCard; products: Product[] } }) => {
    ...
    return {
      meta: [
        { title: main?.title ? `${main.title} — ${shopConfig.name}` : `${shopConfig.name} — ${shopConfig.tagline}` },
        { name: "description", content: loaderData?.card.headline ?? main?.description ?? shopConfig.description },
...
function CardPage() {
  ...
  useEffect(() => {
    ...
    supabase.rpc as any; // placeholder guard
    supabase.from("promo_cards").update({ click_count: (data.card.click_count ?? 0) + 1 }).eq("id", cardId).then(() => {});
  }, [cardId]);
    ...
    return (
      <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-3xl px-4 py-16 text-center">
          ...
          <Link to="/shop" className="mt-6 inline-block rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground">شوف المتجر</Link>
        </div>
        <SiteFooter />
      </div>
    );
      ...
      <section className="mx-auto max-w-6xl px-4 py-8">
        {data.card.headline && (
          <div className="mb-6 rounded-2xl bg-gradient-to-l from-primary/10 to-gold/20 p-6 text-center">
            <h1 className="font-display text-2xl font-bold text-primary md:text-3xl">{data.card.headline}</h1>
          </div>
        )}
          ...
          <div>
            {main.images?.[0] && (
              <div className="aspect-square overflow-hidden rounded-2xl bg-muted">
                <img src={publicImageUrl(main.images[0])} alt={main.title} className="h-full w-full object-cover" />
              </div>
            )}
          </div>
          <div>
            {main.category && <div className="text-xs text-muted-foreground">{main.category}</div>}
            <h2 className="mt-1 font-display text-3xl font-bold text-primary md:text-4xl">{main.title}</h2>
            <div className="mt-3 flex items-baseline gap-1 text-primary">
              <span className="font-display text-3xl font-bold">{Number(main.price).toFixed(0)}</span>
              <span className="text-sm">{shopConfig.currencySymbol}</span>
            </div>
            {main.description && <p className="mt-4 whitespace-pre-line text-sm text-foreground/80">{main.description}</p>}
        ...
        {companions.length > 0 && (
          <div className="mt-16">
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 md:grid-cols-4">
              {companions.map((p) => <ProductCard key={p.id} product={p} />)}
            </div>
          </div>
        )}
      </section>
